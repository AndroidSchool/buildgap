    ...
    droppablesLoop: for (i = 0; i < m.length; i++) {
        ...
        if ((doc = m[i].document[0]) !== document) {
            ...
            if (!frameOffset) {
                ...
                //console.log( $(doc.defaultView || doc.parentWindow).frameElement).scrollTop);
                //console.log("Frame Offset is");
                //TODO make this dependent on the page
                if(ACTIVE_PAGE_BG_UNIQ_ID !== null && ACTIVE_PAGE_BG_UNIQ_ID !== undefined){
                    frameOffset.top = frameOffset.top - $('#device_page_iframe_' + ACTIVE_PAGE_BG_UNIQ_ID).contents().scrollTop();
                }
                //console.log(frameOffset);
                //console.log($('#device_page_iframe_uniq_id_1').contents().scrollTop())
            }
